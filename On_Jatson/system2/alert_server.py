...
역할
----
1. Jetson에서 돌아가는 감지 스크립트(detector_main.py)가 프레임마다 보내주는
   상태(JSON)를 받아서 서버 메모리에 저장한다.
2. 웹 대시보드(dashboard.html)가 WebSocket으로 붙어 있으면 상태가 바뀔 때마다
   실시간으로 밀어준다(push). 즉, 이 서버가 감지 스크립트와 웹 UI 사이의 다리 역할.
3. 경고 단계가 2단계(보호자 알림)에 도달하면 텔레그램으로 보호자 휴대폰에
   실제 메시지를 전송한다.
...
class EngineState:
    ...
    def __init__(self) -> None:
        ...
        self._last_alerted_stage: int = 0  # 이번 방치 사건에서 이미 보낸 최고 단계
        self.history: list[dict] = []  # 최근 알림 로그 (대시보드 표시용)
...
def send_telegram_message(text: str) -> bool:
    """텔레그램으로 보호자 휴대폰에 실제 메시지를 보낸다.

    BotFather에서 만든 봇의 TELEGRAM_BOT_TOKEN과, 보호자가 그 봇과
    대화를 한 번이라도 나눈 뒤 얻은 TELEGRAM_CHAT_ID가 필요하다.
    두 값이 설정되어 있지 않으면 콘솔에만 출력하고 False를 반환한다
    (하드웨어/네트워크가 없는 자리에서도 코드가 죽지 않도록).
    """
