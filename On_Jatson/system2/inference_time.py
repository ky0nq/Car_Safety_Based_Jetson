...
This script DOES NOT import the MiVOLO Python package.
mivolo_v2.pt is loaded directly with torch.jit.load().
"""
...
class MiVOLOTorchScript:
    ...
    def decode(self, output: Any) -> Tuple[float, str, float]:
        """
        Common MiVOLO output:
        [male_logit, female_logit, normalized_age]

        If the exported model uses another output layout, timing remains valid,
        but displayed age/gender can require model-specific decoding.
        """
        age_raw = float("nan")
        gender = "unknown"
...
def main() -> None:
    ...
    try:
        for frame_index in range(1, args.frames + 1):
            ...
            # Total MiVOLO latency above is the sum of all calls in this frame.
            # Divide by the actual call count for a fair per-person/per-call metric.
