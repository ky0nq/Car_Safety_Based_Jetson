...
class SecurityCameraUI:
    ...
    def _draw_button(
        ...
        ty = y1 + (y2 - y1 + th) // 2
        self._put_text(canvas, label, (tx, ty), 0.48, (245, 245, 245), 1)
    ...
    def draw(
        ...
        # Draw recognized faces on the camera portion.
        draw_observations(canvas[:, :width], observations)
        ...
        self._put_text(canvas, f"Camera          /dev/video{camera_index}", (sx + 18, 99), 0.43)
        if status.unknown_presence_elapsed > 0:
            ...
            self._put_text(canvas, alarm_text, (sx + 18, 116), 0.40, (90, 90, 245), 1)

        self._put_text(canvas, "VISIBLE NOW", (sx + 18, 136), 0.44, (165, 165, 165), 1)
        self._put_text(canvas, f"OWNER            {owner_visible}", (sx + 24, 160), 0.46, (100, 235, 100), 1)
        self._put_text(canvas, f"GUEST            {guest_visible}", (sx + 24, 182), 0.46, (235, 190, 100), 1)
        self._put_text(canvas, f"UNKNOWN          {unknown_visible}", (sx + 24, 204), 0.46, (100, 100, 245), 1)
        self._put_text(canvas, f"SPOOF            {spoof_visible}", (sx + 24, 226), 0.46, (220, 100, 230), 1)
        if spoof_visible > 0:
            spoof_text = (
                "SPOOF ALARM SENT"
                if spoof_triggered
                else f"Spoof event in {max(0.0, spoof_seconds - spoof_elapsed):04.1f}s"
            )
            self._put_text(canvas, spoof_text, (sx + 18, 244), 0.38, (220, 100, 230), 1)

        db_owners, db_guests = registered_counts
        self._put_text(canvas, "REGISTERED DB", (sx + 18, 263), 0.44, (165, 165, 165), 1)
        self._put_text(canvas, f"OWNER            {db_owners}/{CONFIG.max_owners}", (sx + 24, 284), 0.46)
        self._put_text(canvas, f"GUEST            {db_guests}/{CONFIG.max_guests}", (sx + 24, 304), 0.46)

        # Buttons. RESET now means clearing the persistent registration list, not the runtime timer.
        button_h = 29
        gap = 5
        x1 = sx + 18
        x2 = sx + PANEL_WIDTH - 18
        y_quit = height - 14 - button_h
        y_reset = y_quit - gap - button_h
        y_list = y_reset - gap - button_h
        y_guest = y_list - gap - button_h
        y_owner = y_guest - gap - button_h
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Vehicle cabin face security camera")
    parser.add_argument("--camera", type=int, default=1, help="Security webcam index")
        ...
        while True:
            ...
            if frame_index % args.process_every == 0:
                ...
                if policy_status.should_alarm:
                    ...
                    ui.toast("WEB ALERT SENT - UNKNOWN 30s", 3.0)
